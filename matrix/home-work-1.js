...
//     let name = new RegExp('^' + roles[i] + '\.');
//
//     play.push(`\n${roles[i]}:\n`);
//
//     for (let j = 0; j < text.length; j++) {
//         if (text[j].match(name)) {
//             text[j] = text[j].replace(name, '').replace(' ', '');
//             play[i] += `${j+1}) ${text[j]}\n`;
//         }
//     }
// }
...
// // const roles = [`Городничий`, `Аммос Федорович`, `Артемий Филиппович`, `Лука Лукич`, 'Христиан Иванович'];
// // let play = [[`\nГородничий:\n`], [`\nАммос Федорович:\n`], [`\nАртемий Филиппович:\n`], [`\nЛука Лукич:\n`], [`\nХристиан Иванович:\n`]];
// // let text2 = `Городничий. Я пригласил вас, господа, с тем чтобы сообщить вам пренеприятное известие: к нам едет ревизор.
// // Аммос Федорович. Как ревизор?
// // Артемий Филиппович. Как ревизор?
// // Городничий. Ревизор из Петербурга, инкогнито. И еще с секретным предписаньем.
// // Аммос Федорович. Вот те на!
...
// //     .split('\n');
// //
// // const text = text2.map((item, index) => `${index + 1}) ${item}\n`);
// //
// // for (let i = 0; i < roles.length; i++) {
// //     if (roles[i] === 'Христиан Иванович') replaceTxt(i,/^\d+\) Христиан Иванович/, 'Христиан Иванович');
// //     else replaceTxt(i, `${roles[i]}\. `, `${roles[i]}\. `);
// // }
// //
// // play = play.map(item => item.join(' '));
// //
// // console.log(play.join(' '));
// //
// // // Техническая
// // function replaceTxt (elem, regEx, replaceRegEx) {
// //     let name = new RegExp(regEx);
// //
// //     for (let j = 0; j < text.length; j++) {
// //         if (text[j].match(name)) {
// //             text[j] = text[j].replace(replaceRegEx, '');
// //             play[elem].push(text[j]);
// //         }
// //     }
// // }

// // II. Заполнить матрицу спиралью
// const w = +prompt('Ширина матрицы', ''),
//     h = +prompt('Высота матрицы', '');
//
// function matrix (w, h) {
